...
use regex::Regex;
use rspack_core::{
  compare_runtime, BoxModule, Chunk, ChunkGraph, ChunkUkey, Compilation, Module, ModuleGraph,
  ModuleIdentifier,
};
use rspack_util::{
...
fn compare_chunks_by_modules(
  ...
  let a_modules = chunk_graph.get_chunk_modules_iterable(&a.ukey, module_graph);
  let b_modules = chunk_graph.get_chunk_modules_iterable(&b.ukey, module_graph);
